...
两台设备均作为根节点连接路由器但未组网，如何解决？
-------------------------------------------------------------------

  常见原因是两台设备的 Mesh ID 不一致。Mesh-Lite 组网要求 Mesh ID 与路由器信息（SSID、密码）一致。

--------------

Mesh-Lite 使用 APP 配网，已连接 Wi-Fi 1 再切换到 Wi-Fi 2， ``esp_restart`` 后仍连接 Wi-Fi 1，如何解决？
----------------------------------------------------------------------------------------------------------------------------------------------------

  在调用 ``esp_restart`` 之前执行 ``esp_mesh_lite_erase_rtc_store``，清除 RTC 中存储的组网信息即可。

--------------

Mesh-Lite 子节点可以直接维护 MQTT 长连接吗？
-----------------------------------------------

  可以，无需先将数据发给根节点，子节点可以直接维护 MQTT 长连接。
