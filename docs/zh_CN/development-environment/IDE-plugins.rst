...
------------------------------------------------------------------------------------------------
  ...
  - 请前往 ``File -> Preferences -> Show verbose output during`` 勾选 ``compilation``，编译成功后会打印一条 python 烧录命令，其中包含待烧录的 bin 文件以及对应的烧录地址。
  - 在乐鑫官网的 `工具页面 <https://www.espressif.com/zh-hans/support/download/other-tools>`_ 下载 Flash 下载工具，使用 Flash 下载工具烧录时选择 bin 文件，输入对应的烧录地址即可。
