...
使用 ESP32-S3-WROOM-1 模组，是否可以复用乐鑫的 FCC/CE 认证报告，而不需要额外做定频测试？
------------------------------------------------------------------------------------------------------------------------------------------------------------------

  可以复用部分数据，但整机测试需要补测 EMC。

--------------

先用蓝牙配网，再关闭蓝牙后测 Wi-Fi，信令测试结果比单独测 Wi-Fi 差，是什么原因？
------------------------------------------------------------------------------------------

  蓝牙不会影响 Wi-Fi 的实际性能，但会影响信令测试流程。若测试 Wi-Fi 时蓝牙仍占用射频前端资源，Wi-Fi 会丢包，进而影响信令测试过程中仪器与芯片的交互，使测得数据出现偏差，而非实际 RF 性能下降。测试时需确保蓝牙已完全关闭（deinit 彻底）。
