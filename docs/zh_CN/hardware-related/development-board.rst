...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  - 降低功耗：如果摄像头并非实时开启，Wi-Fi 可以周期性传输，空闲时间可以进入休眠模式以降低功耗。
...
------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  建议检查开发板是否有 Reset 按键，由于 EN 常用作复位功能，部分开发板丝印会标记为 Reset 按键。
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  AEC 回声参考信号的硬件回采电路将 Codec (ES8311) 的 DAC 输出同时传输给喇叭 PA 和 ADC (ES7243) AINLP/N，随后将采集的信号送回 ESP32，用作 AEC 回声消除算法的参考信号。

----------------

使用 `ESP32-MINI-1 <https://www.espressif.com/sites/default/files/documentation/esp32-mini-1_datasheet_cn.pdf>`__ 模组，上电后串口打印以下两类日志，是什么原因？
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  以上两类日志均表示 ESP32-MINI-1 模组的 flash 尚未烧录固件。不同 ECO 版本的 ROM Bootloader 提示方式不同：ESP32 (ECO V3) 打印 ``invalid header: 0xffffffff``，ESP32 (ECO V1) 打印 ``flash read err, 1000``。

  如果模组尚未烧录固件，这是正常现象。向 flash 烧录固件后，重新上电即可正常启动。
