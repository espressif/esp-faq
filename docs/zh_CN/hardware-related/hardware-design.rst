...
------------------------------------------
  ...
  请参考文档 `RTC 时钟源（可选）(ESP32) <https://docs.espressif.com/projects/esp-hardware-design-guidelines/zh_CN/latest/esp32/schematic-checklist.html#rtc>`_ 中的硬件设计说明。
...
--------------------------------------------------------------------------------------------------------------------------------------
  ...
  - 若需要使用 GPIO12 用作其他功能，请使用 esptool 工具通过 `espefuse.py set_flash_voltage 3.3V <https://docs.espressif.com/projects/esptool/en/latest/esp32/espefuse/set-flash-voltage-cmd.html#set-flash-voltage>`__ 命令将 VDD_SDIO 固定为 3.3 V。
  - 硬件上可以将 VDD_SDIO 直接连到 3.3 V 上，这样就不用再烧录 eFuse。
  - 在量产阶段，也可以直接将 flash 下载工具里 ``config/esp32/utility.config`` 文件下 ``ESP32_EFUSE_CONFIG`` 的默认配置选项修改为 ``config_voltage = 3.3 V`` 来下载固件。
...
-----------------------------------------------------------------------------------------------------------------------
  ...
  不支持。详细说明参见文档 `RTC 时钟源（可选）(ESP32-C3) <https://docs.espressif.com/projects/esp-hardware-design-guidelines/zh_CN/latest/esp32c3/schematic-checklist.html#rtc>`_ 中的硬件设计说明。
...
------------------------------------------------------------------------------------------------------------------------
  ...
  U0TXD 预留 499 Ω 电阻是用于抑制 80 MHz 谐波。详细信息请参考文档 `UART <https://docs.espressif.com/projects/esp-hardware-design-guidelines/zh_CN/latest/esp32s3/schematic-checklist.html#uart>`__。
...
哪些产品支持使用备用电池为 RTC 单独供电？
-------------------------------------------------------------------------------------------------

  - 目前，ESP32-H2 和 ESP32-P4 均支持通过 VBAT 管脚为 RTC 电源域提供备用供电。
  - 可通过 ``CONFIG_ESP_VBAT_INIT_AUTO`` 配置项启用，兼容电池电压范围如下：

    - ESP32-P4：2.3 V ~ 3.6 V
    - ESP32-H2：2.5 V ~ 3.6 V

  - 注意：若使用 ESP32-H2 模组，VBAT 管脚在模组内部已连接 3.3 V，实际不可用。
  - 测试例程请参见 `esp-idf/examples/lowpower/vbat <https://github.com/espressif/esp-idf/tree/v6.0.2/examples/lowpower/vbat>`__。
  - 特别说明：

    - 使用 VBAT 作为 RTC 电源域的备用供电时，硬件必须外接 32.768 kHz 晶振。
    - 芯片仅在进入 Deep-sleep 模式后才会自动切换到 VBAT 为 RTC 供电；在 Light-sleep 模式下不支持自动切换。
    - 不支持在主电源意外掉电时，由芯片内部自动切换到 VBAT 以保持 RTC 计时。
