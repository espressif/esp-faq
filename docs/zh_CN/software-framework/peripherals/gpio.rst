...
------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  可以混合设置。例如 VDD_IO_4 域的 GPIO 使用 1.8 V，而 VDD_IO_6 域的 GPIO 使用 3.3 V。但是同一电源域下的 GPIO 引脚的电平必须统一，要么全部是 1.8 V，要么全部是 3.3 V。例如，如果 VDD_IO_4 输入 1.8 V 电压，则该电源域下的所有 GPIO 电平均为 1.8 V。
