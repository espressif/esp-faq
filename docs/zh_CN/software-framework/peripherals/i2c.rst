...
----------------------------------------------------------------------------------------------------------------------------------------------
  ...
  可以。在 ``esp_driver_i2c`` 驱动中，可以选择 ``LP_I2C_PORT`` 以启用 LP I2C 功能。具体实现请参考例程：`esp-idf/components/esp_driver_i2c/test_apps/i2c_test_apps/main/test_lp_i2c.cpp <https://github.com/espressif/esp-idf/blob/master/components/esp_driver_i2c/test_apps/i2c_test_apps/main/test_lp_i2c.cpp>`_
...
ESP32 芯片在进行 I2C 读写时，为什么第 8 和第 9 个时钟 (CLK) 之间会有延迟？
---------------------------------------------------------------------------------------------------------------------

  这是 ESP32 硬件设计的限制，硬件需在此期间处理特定内部逻辑。其他系列芯片无此问题。
