...
设备量产后遇到 TLS 证书过期，应如何处理？
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  - 尽可能延长证书的有效期，保证证书有效期大于设备使用年限；
  - 在证书过期之前，通过 OTA 的方式更新证书；
  - 若证书已过期，可以通过其他远程方式更新证书，例如通过 HTTP 接口上传新证书。

----------------

在 TLS 或 WSS 连接中提示证书错误，但确认客户端配置无误，如何定位问题？
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  可使用其他 TLS/WSS 客户端（如 PC 或第三方库）连接同一服务器进行对比测试。如果同样报证书错误，通常说明服务器返回的证书链或证书本身存在问题，而非设备端验证逻辑错误。
