...
通过 HTTP 下载固件时出现读取超时，有时在读取约 5 KB 数据后发生（默认 TCP 接收窗口 5760），延长超时时间也无法解决。可能是什么原因？如何解决？
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  可能原因：

    当前运营商基站工作策略特殊，乐鑫设备未按整包回复 ACK 导致中间网络设备丢弃报文，此时服务器收不到 ACK 也不再继续发送数据。

  排查思路：

    1. 更换运营商网络测试。
    2. 检查并增大 TCP 接收窗口大小（如 ``CONFIG_LWIP_TCP_WND_DEFAULT``），观察失败时已读取数据量是否随窗口增大而增加。
    3. 联系乐鑫技术支持，确认针对性修复方法。


--------------

使用 ``esp_http_client_open`` 等底层 API 进行 POST 时，如何保持 HTTP 连接复用？
------------------------------------------------------------------------------------
