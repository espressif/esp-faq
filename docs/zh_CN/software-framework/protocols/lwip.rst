...
设备为何会打开多个 UDP 端口？是否正常？ 
------------------------------------------------------------------------------------------------------------------

  是正常的。UDP 端口用于 DHCP、DNS、SNTP 等协议，每次查询可能会临时分配一个端口（1024 以上），使用完毕后会关闭。

-----------------

ESP32-P4 使用以太网连接时无法获取全局 IPv6 地址，但可以获得链路 IPv6 地址，应如何处理？
------------------------------------------------------------------------------------------------------------------

  可以查看以太网场景下，`esp_netif_lwip.c 文件的 1176 行 <https://github.com/espressif/esp-idf/blob/v5.5.1/components/esp_netif/lwip/esp_netif_lwip.c#L1176>`_ 是否被执行，同时确认 ``ip6_autoconfig_enabled`` 的值为 0 还是 1。

-----------------

设备有多个 netif（如 Wi-Fi 和以太网）时，能否在一个 netif 断网后自动将数据切换到另一个 netif？
------------------------------------------------------------------------------------------------------------------

  不支持自动切换。数据流向由 netif 的优先级决定。如果高优先级的 netif 仍保持连接状态（即使实际断网），数据仍会尝试从该接口发出，不会自动切换到另一个可用的低优先级接口。

-----------------

设备同时启用 Wi-Fi 和以太网，为以太网设置静态 IP 后，DNS 解析失败。应如何排查和解决？
------------------------------------------------------------------------------------------------------------------

  多 netif 场景下，后启动的 netif 获取的 DNS 服务器可能会覆盖之前的。解决方案如下：

  1. 检查并开启 ``LWIP_DNS_SETSERVER_WITH_NETIF`` 配置，使每个 netif 拥有独立的 DNS 服务器。
  2. 确保设置静态 IP 的代码在正确的网络事件（如 ``ETHERNET_EVENT_CONNECTED``）之后执行。

-----------------

如何控制 mDNS 服务在特定的网络接口（netif）上发布？例如让不同服务分别发布在 Wi-Fi 或以太网接口上？
------------------------------------------------------------------------------------------------------------------

  当前 mDNS 实现不支持为不同 netif 注册不同的服务。服务是全局注册的，只能控制哪个 netif 接口启用或禁用 mDNS 功能（通过 ``mdns_netif_action``），但不能实现“A 服务仅 WiFi 发布，B 服务仅以太网发布”的需求。

-----------------

使用 esp_websocket_client 时，如何在自动重连前更新 headers（如认证令牌）？
------------------------------------------------------------------------------------------------------------------

  较新版本的 ``esp_websocket_client`` 组件提供了 ``esp_websocket_client_set_headers`` API，可以在需要时动态更新头部信息，无需销毁重建 client。

--------------

ESP32 系列芯片在 IPv6 模式下，global IPv6 地址前缀获取、DNS server 地址获取等场景是否都支持？
----------------------------------------------------------------------------------------------------------------------------------------------------

  global IPv6 地址前缀获取只支持 SLAAC 方式，DNS server 地址获取可以支持 SLAAC 和 DHCPv6 stateless 两种方式。

--------------

ESP-IDF 是否支持 SNMP 协议栈？
-------------------------------

  lwIP 里内置了 SNMPv3 支持，但 ESP-IDF 上层没有启用也没有提供示例。用户需自行调用 lwIP 的 SNMP 接口。

--------------

使用以太网接收 UDP 数据包发现乱序（存在丢包），如何判断是 lwIP 层丢包还是以太网层丢包？
-----------------------------------------------------------------------------------------------------

  将 ``CONFIG_LWIP_TCPIP_RECVMBOX_SIZE`` 和 ``CONFIG_LWIP_UDP_RECVMBOX_SIZE`` 调小（如从 48 改到 16），若收到的数据包也相应减少就说明是 lwIP 层丢包；否则问题在以太网层。
