...
使用了经 OpenSSL 验证合法的证书，设备端连接 MQTT 服务器时遇到验证证书失败，而使用 MQTTX 工具可以成功连接和验证，是什么原因？
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  可检查证书中的 SAN（Subject Alternative Name，主体别名）IP 地址字段长度是否满足 x.509 标准定义。Mbedtls 里针对 IPv4 的长度是 4 字节，IPv6 的长度是 16 字节。

-------------

WolfSSL 的授权是否包含在乐鑫芯片中？
--------------------------------------------------------------------------------------------------------------------------------------------

  ESP 芯片与 WolfSSL 的协议已经到期，用户需要自行购买授权许可才能商用。

-------------

OTA 升级过程中出现 Mbedtls 错误（如 -0x7780），如何解决？
--------------------------------------------------------------------------------------------------------------------------------------------

  0x7780 表示设备收到对端发送的警告消息。例如，当乐鑫设备采用自签的 CA 证书并充当 HTTP 服务器时，如果有 PC 浏览器访问该服务器，电脑端就会发送该警告消息，但不影响正常的通信。其他情况下，最好打开 Mbedtls 日志看下问题。

-------------

TLS 握手失败，错误码 -0x7100 (BAD_INPUT_DATA)，可能是什么原因？
--------------------------------------------------------------------------------------------------------------------------------------------

  这通常是由于接收到的数据不完整，从而无法进行解析造成的。确认下 ``CONFIG_MBEDTLS_SSL_IN_CONTENT_LEN`` 是不是 16 K，如果已经设置成最大 16 K 了，建议打开 Mbedtls 日志看下问题。

--------------

证书在 MQTTX 上可以正常连接本地 MQTT broker，但在乐鑫设备上报错 ``mbedtls_ssl_handshake returned -0x7A00``，是证书格式存在问题吗？
---------------------------------------------------------------------------------------------------------------------------------------------------------------

  错误码 ``-0x7A00`` 指向证书校验失败，不一定是格式问题。排查建议：
  1. 开启 mbedTLS 日志。
  2. 在 ESP-IDF 中启用 ``esp_mbedtls_verify_certificate`` 函数中的 debug 日志以获取详细失败原因。

---------------

ESP32-C3 使用 Curve25519 解密 APP 发送的路由器信息时出现乱码，该芯片是否支持此算法？
------------------------------------------------------------------------------------------------------------------------

  Mbed TLS 支持 Curve25519 算法，且配置项 ``CONFIG_MBEDTLS_ECP_DP_CURVE25519_ENABLED`` 默认已使能。建议检查数据的大小端转换以及加解密逻辑。你可以分别在设备侧和 APP 侧打印生成的 AES 共享密钥，对比两者是否一致。
