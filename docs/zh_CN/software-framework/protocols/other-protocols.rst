...
ESP32-P4 能否支持运行 SSH 客户端和服务器？是否有官方组件？
------------------------------------------------------------------------------------------------------------------------

  软件层可以运行。因为 SSH 不依赖特定硬件，所以 ESP32 系列芯片均可支持。目前官方尚未提供专门的 SSH 组件适配，但你可以参考 `乐鑫组件注册表 <https://components.espressif.com/>`_ 上的第三方组件。
