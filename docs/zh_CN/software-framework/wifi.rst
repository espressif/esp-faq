...
----------------------------------------
  ...
  对于 5 GHz 信道，根据监管规则，DFS 信道需要采用被动扫描。不同国家/地区的 DFS 信道不同，可参考 `esp_wifi_regulatory.c <https://github.com/espressif/esp-idf/blob/055ba9d3f9c6fd9a0efacd4993a2a942972dd65d/components/esp_wifi/regulatory/esp_wifi_regulatory.c#L280>`__：其中 DFS 标志位为 1 的即为 DFS 信道。
...
-----------------------------------------------
  ...
  - 一共有三种休眠方式: Modem sleep, Light sleep 和 Deep sleep。
    ...
    - Modem sleep: Wi-Fi 协议规定的 station WMM 休眠方式(station 发送 NULL 数据帧通知 AP 休眠或醒来)，station 连接上 AP 之后自动开启，进入休眠状态后关闭射频模块，休眠期间保持和 AP 的连接，station 断开连接后 modem sleep 不工作。ESP32 modem sleep 进入休眠状态后还可以选择降低 CPU 时钟频率，进一步降低电流。
    - Light sleep: 基于 modem sleep 的 station 休眠方式，和 modem sleep 的不同之处在于进入休眠状态后不仅关闭射频模块，还暂停 CPU，退出休眠状态后 CPU 从断点处继续运行。
    - Deep sleep: 非 Wi-Fi 协议规定的休眠方式，进入休眠状态后关闭除 RTC 模块外的所有其他模块，退出休眠状态后整个系统重新运行(类似于系统重启)，休眠期间不能保持和 AP 的连接。
...
---------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  - 前两位表示的原因可以从 `Wi-Fi Reason Code <https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32/api-guides/wifi.html#id35>`_ 里查看。后两位可以直接从管理帧代码里查看。
...
ESP 模块支持 Wi-Fi NAN (Neighbor Awareness Networking) 协议吗？
------------------------------------------------------------------------------------------------
...
在 Wi-Fi Station 模式下，当路由器同时开启 Wi-Fi 4 和 Wi-Fi 6 模式时，ESP32-C6 将使用哪种 Wi-Fi 模式与路由器建立连接？
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  当未限制 ESP32-C6 的协议模式时，ESP32-C6 将优先使用 Wi-Fi 6 与路由器建立连接。也可以通过 `esp_wifi_set_protocol() <https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32c6/api-reference/network/esp_wifi.html#_CPPv421esp_wifi_set_protocol16wifi_interface_t7uint8_t>`__ 限制是否使用 Wi-Fi 6。

-----------------

在 Wi-Fi AP 模式下，ESP32-C2 系列产品最多支持几个 Wi-Fi Station 设备连接？
-------------------------------------------------------------------------------------------------------------------------------------------------

  - 由于硬件限制，ESP32-C2 在 Wi-Fi AP 模式下最多支持四个 Wi-Fi Station 连接。
  - 不同型号的芯片在 Wi-Fi AP 模式下支持的最大连接数不一样，详情请参阅 `esp-idf/components/esp_wifi/include/esp_wifi_types.h <https://github.com/espressif/esp-idf/blob/a322e6bdad4b6675d4597fb2722eea2851ba88cb/components/esp_wifi/include/esp_wifi_types.h#L379>`_。

---------------

ESP32 支持 WPA3 Wi-Fi AP 模式吗？
-----------------------------------------------------------------------------------------------------------------------

  支持。ESP-IDF v5.1 及以上版本支持 WPA3 Wi-Fi AP 模式，参见 `ESP-IDF v5.1 Release <https://github.com/espressif/esp-idf/releases/tag/v5.1>`_ 中的说明。
...
基于 Espressif 系列的产品启用 Wi-Fi AP 模式时，是否支持设置自动信道？
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  Espressif 系列的产品是 Wi-Fi 软 AP (SoftAP) 模式，不支持设置自动信道。
...
路由器的 DHCP 租期时长会影响模块的 Wi-Fi 连接吗？
-----------------------------------------------------------------------------------------------------------------------

  DHCP 协议会自动负责租约续期，无需重启设备或 Wi-Fi。如果续约失败，模块可能会丢失 IP 地址，此时需要通过软件逻辑检测并重新获取 IP 地址。

-------------

如果将模块的认证模式设置为 WIFI_AUTH_OPEN，但将路由器设置为更高安全性的认证模式（如 WIFI_AUTH_WPA3_PSK），模块是否仍能成功连接？
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  是的，模块可以成功连接到路由器，因为设置的最低认证模式 (wifi_sta_config_t threshold.authmode) 只要求路由器的加密等级不低于此值即可。自 IDF v5.0 版本以后，如果需要连接 WEP 或 WPA 网络，必须将 authmode 设置为 ``WIFI_AUTH_WEP`` 或 ``WIFI_AUTH_WPA_PSK``。

-------------

遇到 esp-aes: Failed to allocate memory 问题时，应该如何处理？
-----------------------------------------------------------------------------------------------------------------------
...
ESP32-C6 的 Wi-Fi 6 模式和 HT40 之间的共存关系是怎样的？
--------------------------------------------------------------------------------------------------------------------------------------------
...
使用 ESP32 设备时，是否可以在保持 Wi-Fi 连接的同时开启 smartconfig 配网 (esp_smartconfig_start)，以便随时切换到其他 Wi-Fi？
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------

  不支持。开启 smartconfig 配网 (esp_smartconfig_start) 后会进入 promiscuous 模式并切换信道，这会导致之前的 Wi-Fi 连接断开。
...
Wi-Fi STA 发送数据时 AP 未收到 ACK 可能是什么原因？
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  如果 AP 未收到 STA 发送的 ACK，可能是由于 Wi-Fi 信号问题或 STA 的 TX 缓冲区已满。建议检查 Wi-Fi 信号强度，并确保 STA 的 TX 缓冲区有足够的空间。此外，还可以通过抓取 Wi-Fi 空口包来进一步分析问题。

---------------

Wi-Fi STA 日志中出现 ``wifi:bcn_timeout,ap_probe_send_start`` 时，为什么功耗会上升？
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
ESP32-C5 能否在 20 MHz 下嗅探到 Wi-Fi 5 和 Wi-Fi 6 的数据包？
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  是的，ESP32-C5 可以在 20 MHz 时嗅探到 Wi-Fi 5 和 Wi-Fi 6 的数据包，前提是这些数据包在同一个信道上传输。如果数据包所在的信道不同，则无法嗅探到。

---------------

为什么 ESP32 通过 4G 拨号 ping 公网 IP 时会超时？
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  可能的原因：

  - 运营商网络波动，建议尝试更换不同的网络进行测试。
  - AP 模式可能影响转发。如果 ESP32 开启了 AP 模式并有终端设备连接，默认路由会从 4G 网络切换为 AP 网卡。建议关闭 AP 模式进行验证。

---------------

在局域网内，能否通过 UDP 数据内容查找手机的 MAC 地址而不使用 ARP？
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  不建议这样做，因为点对点通信必须知道对端的 MAC 地址，这是 ARP (IPv4) 和 NDP (IPv6 ICMPv6) 存在的意义。自行实现的方式手机端不会支持。
...
乐鑫芯片是否支持 ARP 老化机制？老化时间是多久，是否可以修改？例如，Wi-Fi 连接时会通过 ARP 获取 AP 的 MAC, 间隔多久后会再次发送 ARP 请求?
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  - 支持 ARP 老化机制。
  - 默认老化时间为 5 分钟，默认宏为 `ARP_MAXAGE`。目前未提供 API 或 menuconfig 配置项来修改该时间。
  - 访问某个 IP 地址时，芯片会首先查询 ARP 表，如果 ARP 表中没有或即将过期的条目，会发送 ARP 请求。收到 ARP 响应后，芯片会更新 ARP 表，并维持该记录有效约 5 分钟。

---------------

``LWIP_GARP_TMR_INTERVAL`` 是做什么的？发送 ARP 的的间隔是多少秒？
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  对于 ``LWIP_GARP_TMR_INTERVAL``，GARP 是乐鑫芯片主动发送 IP 和 MAC 地址映射关系的机制，用于通知其他设备更新其 ARP 表。INTERVAL 代表主动发送的时间间隔，默认值为 60 秒。

---------------

关闭 modem sleep 后，ping 延迟仍然较高，如何进一步优化？
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  - 在开放无线环境中，Wi-Fi 延迟受信道竞争和空口波动影响，lwIP 层面的优化效果有限。

  - 可以执行以下操作优化：

    1.  尝试关闭 WIFI AMPDU 功能；
    2.  根据芯片型号参考 `Wi-Fi 驱动程序 <https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32/api-guides/wifi.html#how-to-improve-wi-fi-performance>`_ 中的相关文档，尝试提升 Wi-Fi 性能。

  - 如果对端同样是 ESP 设备且时延要求极高，建议采用 `ESP-NOW <https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32/api-reference/network/esp_now.html>`_ 等直连协议以降低时延。

---------------

使用抓包工具分析 Wi-Fi 数据时，输入了 SSID 和密码但数据仍未解密，是否正常？
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  正常。

  - 如果路由器的加密方式为 WPA2，除了账号密码外，还需要捕获完整的 eapol 1 ~ eapol 4 握手过程，若未捕获完整的四次握手过程，抓包工具无法解密后续数据。需要重新进行网络连接并进行抓包，以获取完整握手数据。
  - 如果路由器的加密方式为 WPA3，除了账号密码和完整的握手过程外，还需要获取 PMK 才能解密，PMK 可以通过使能 ``ESP_WIFI_DEBUG_PRINT`` 从打印中获取。

---------------

ESP32-C5 支持 5GHz 频段做 AP 热点吗？2.4G 和 5G 做 AP 时 MAC 地址是否相同？
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

  - ESP32-C5 支持在 5GHz 频段工作于 AP 模式。在 IEEE 802.11n 下支持 HT40 带宽；在 IEEE 802.11ac 和 IEEE 802.11ax 下不支持 40 MHz 带宽。在 802.11ax 模式下，仅支持 HE 速率和 BSS Color 功能，不支持 OFDMA、MU-MIMO 等其他特性。此外，C5 当前不支持双频同时工作，任一时刻仅能工作在单一频段。
  - 在 AP 模式下，2.4 GHz 和 5 GHz 共用同一个 MAC 地址。

---------------

ESP 芯片是否支持 Wi-Fi Direct / Wi-Fi P2P？
------------------------------------------------------------------------------------------------------------------------

  目前乐鑫芯片不支持 Wi-Fi Direct 或 Wi-Fi P2P，且暂无计划。

---------------

ESP32-C5 是否支持在 5G 频段上使用 ESP-NOW？
------------------------------------------------------------------------------------------------------------------------
